...
public class MemberRegistrationController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<String> signup(@Valid @RequestBody MemberRequestForm memberRequestForm,
        ...
        if (!memberRequestForm.getPassword1().equals(memberRequestForm.getPassword2())) {
        ...
        try {
            memberRegistrationService.create(memberRequestForm);
...
//
//        try {
//            Resident resident = memberRegistrationService.createResident(memberRequestForm);
//        } catch (DataIntegrityViolationException e) {
//            e.printStackTrace();
//            bindingResult.reject("signupFailed", "이미 등록된 사용자입니다.");
//            return ResponseEntity.ok("ok");
...
//    }
    ...
    public ResponseEntity<String> updateAccount(@Valid @RequestBody MemberRequestForm requestForm, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            StringBuilder errorMessage = new StringBuilder("Validation failed. ");
        ...
        if (!requestForm.getPassword1().equals(requestForm.getPassword2())) {
            bindingResult.rejectValue("password2", "passwordInCorrect",
                    "2개의 패스워드가 일치하지 않습니다.");
        ...
        try {
            memberRegistrationService.update(requestForm);
        } catch (Exception e) {
            e.printStackTrace();
